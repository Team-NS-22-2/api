...
public class PaymentDaoImpl extends Dao implements PaymentDao {
    ...
    public Payment read(int id) {
        ...
        if (payment == null) {
            throw new MyIllegalArgumentException(ConsoleColors.RED_BOLD+id + "는 없는 ID 입니다. 다시 확인해주세요."+ ConsoleColors.RESET);
        }
    ...
    public List<Payment> findAllByCustomerId(int customerId) {
        ...
        if(paymentList.isEmpty())
            throw new MyIllegalArgumentException(CustomerViewLogicConstants.NO_PAYMENT_ON_CUSTOMER);
        return  paymentList;
    }
