...
public class CustomerService {
    ...
    public void registerHealthContract(int insId, RegisterHealthContractRequest request) {
        ...
        if(insuranceType != InsuranceType.HEALTH) return;
    ...
    public void registerFireContract(int insId, RegisterFireContractRequest request) {
        Customer customer = customerInstance.registerCustomer(request.getCustomerDto());
        customer.setId(customerRepository.save(customer).getId());
        InsuranceType insuranceType = insuranceRepository.findInsuranceTypeByInsuranceId(insId).orElseThrow(NullDataException::new);
        if(insuranceType != InsuranceType.FIRE) return;
    ...
    public void registerCarContract(int insId, RegisterCarContractRequest request) {
        Customer customer = customerInstance.registerCustomer(request.getCustomerDto());
        customer.setId(customerRepository.save(customer).getId());
        InsuranceType insuranceType = insuranceRepository.findInsuranceTypeByInsuranceId(insId).orElseThrow(NullDataException::new);
        if(insuranceType != InsuranceType.FIRE) return;
